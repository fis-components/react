...
function extractObjectFromArray(arr, keyExtractor) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    !!(normalizedKey in normalizedObj) ? true ? invariant(false, 'OrderedMap: IDs returned by the key extraction function must be unique.') : invariant(false) : undefined;
...
function assertValidPublicKey(key) {
  !(key !== '' && (typeof key === 'string' || typeof key === 'number')) ? true ? invariant(false, 'OrderedMap: Key must be non-empty, non-null string or number.') : invariant(false) : undefined;
}
...
function assertValidRangeIndices(start, length, actualLen) {
  !(typeof start === 'number' && typeof length === 'number' && length >= 0 && start >= 0 && start + length <= actualLen) ? true ? invariant(false, 'OrderedMap: `mapRange` and `forEachRange` expect non-negative start and ' + 'length arguments within the bounds of the instance.') : invariant(false) : undefined;
}
...
function _fromNormalizedObjects(a, b) {
  ...
  !(a && a.constructor === Object && (!b || b.constructor === Object)) ? true ? invariant(false, 'OrderedMap: Corrupted instance of OrderedMap detected.') : invariant(false) : undefined;
...
var OrderedMapMethods = {
  ...
  merge: function (orderedMap) {
    !(orderedMap instanceof OrderedMapImpl) ? true ? invariant(false, 'OrderedMap.merge(...): Expected an OrderedMap instance.') : invariant(false) : undefined;
  ...
  mapKeyRange: function (cb, startKey, endKey, context) {
    ...
    !(startIndex !== undefined && endIndex !== undefined) ? true ? invariant(false, 'mapKeyRange must be given keys that are present.') : invariant(false) : undefined;
    !(endIndex >= startIndex) ? true ? invariant(false, 'OrderedMap.mapKeyRange(...): `endKey` must not come before `startIndex`.') : invariant(false) : undefined;
  ...
  forEachKeyRange: function (cb, startKey, endKey, context) {
    var startIndex = this.indexOfKey(startKey);
    var endIndex = this.indexOfKey(endKey);
    !(startIndex !== undefined && endIndex !== undefined) ? true ? invariant(false, 'forEachKeyRange must be given keys that are present.') : invariant(false) : undefined;
    !(endIndex >= startIndex) ? true ? invariant(false, 'OrderedMap.forEachKeyRange(...): `endKey` must not come before ' + '`startIndex`.') : invariant(false) : undefined;
  ...
  nthKeyAfter: function (key, n) {
    ...
    !(curIndex !== undefined) ? true ? invariant(false, 'OrderedMap.nthKeyAfter: The key `%s` does not exist in this instance.', key) : invariant(false) : undefined;
...
var OrderedMap = {
  from: function (orderedMap) {
    !(orderedMap instanceof OrderedMapImpl) ? true ? invariant(false, 'OrderedMap.from(...): Expected an OrderedMap instance.') : invariant(false) : undefined;
    return _fromNormalizedObjects(orderedMap._normalizedObj, null);
  },

  fromArray: function (arr, keyExtractor) {
    !Array.isArray(arr) ? true ? invariant(false, 'OrderedMap.fromArray(...): First argument must be an array.') : invariant(false) : undefined;
    !(typeof keyExtractor === 'function') ? true ? invariant(false, 'OrderedMap.fromArray(...): Second argument must be a function used ' + 'to determine the unique key for each entry.') : invariant(false) : undefined;
